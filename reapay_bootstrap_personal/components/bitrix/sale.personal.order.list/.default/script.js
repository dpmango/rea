...
$(function(){
    ...
    $('.seefullinfo').bind('click',function(){
        $(this).siblings('.fullinfo').fadeIn();
        var see = $(this);
        setTimeout(function(){
            see.siblings('.fullinfo').fadeOut();
        }, 5000);
    });


    if ($(event.target).closest(".fullinfo").length || $(event.target).closest(".seefullinfo").length) return;
    $(".fullinfo").fadeOut();
