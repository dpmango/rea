...
window.payment_percent = 0;
$(window).ready(function(){
    console.log('here');
    $('.payment_type').find('input[type=radio]').change(function() {

        window.payment_percent =  parseInt(this.value);

        $('.bx-soa-percent').find('.bx-soa-cart-d').html(window.payment_percent+'%');

        var $totalSumBlock = $('.bx-soa-cart-total-line-total').find('.bx-soa-cart-d').eq(1);

        if ( parseInt( $totalSumBlock.data('total') ) > 0 )
        	var iCurrentTotal = parseInt( $totalSumBlock.data('total') );
		else
        	var iCurrentTotal = parseInt($totalSumBlock.text().replace(/\D/g,''));


        $totalSumBlock.data('total',iCurrentTotal);

        var iTotalWithPercent = numberWithSpaces( parseInt( (iCurrentTotal/100)*window.payment_percent) + iCurrentTotal );
        $totalSumBlock.text( iTotalWithPercent + ' руб. '  );
