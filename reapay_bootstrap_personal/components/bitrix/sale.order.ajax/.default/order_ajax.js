...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		editOrder: function()
		{
			...
			console.log('this.result.SHOW_AUTH');
			console.log( this.result.SHOW_AUTH  );

			this.orderSaveBlockNode.style.display = 'block'; //this.result.SHOW_AUTH ? 'none' : '';
			this.mobileTotalBlockNode.style.display = 'block';//this.result.SHOW_AUTH ? 'none' : '';
            ...
            this.totalBlockNode.innerHTML += '<div class="rightmenu">'
                +'<br><br><div class="rightmenu">'
					+'<div class="payments">'
					+'<img src="/local/templates/reapay_bootstrap/images/pay2.png" alt="">'
					+'<img src="/local/templates/reapay_bootstrap/images/pay1.png" alt="">'
					+'<img src="/local/templates/reapay_bootstrap/images/pay4.png" alt="">'
					+'<img src="/local/templates/reapay_bootstrap/images/pay3.png" alt="">'
					+'<img src="/local/templates/reapay_bootstrap/images/pay5.png" alt="">'
					+'<img src="/local/templates/reapay_bootstrap/images/pay6.png" alt="">'
					+'</div>'
					+'<ul>'
					+'<li><a href="/payment_security.php">Информация о безопасности платежей</a></li>'
					+'<li><a href="/payment_rules.php">Порядок оплаты банковскими картами</a></li>'
					//+'<li><a href="#">Порядок и адреса выставления претензий и возврата товара покупателям</a></li>'
					+'</ul>'
					+'</div>';

            this.basketBlockNode.innerHTML += '<div class="billfooter">'
		...
		editPaySystemBlock: function(active)
		{
			...
			/*
			 node.innerHTML += '<p><strong>Направление</strong>: '+arContractInfo.PROPS.DIRECTION+'</p>';
			 node.innerHTML += '<p><strong>Номер договора</strong>: '+arContractInfo.PROPS.NUMBER+'</p>';
			 node.innerHTML += '<p><strong>Обучающийся</strong>: '+arContractInfo.PROPS.STUDENT_FIO+'</p>';
			 node.innerHTML += '<p><strong>Уровень образования</strong>: '+arContractInfo.PROPS.LEVEL+'</p>';
			 node.innerHTML += '<p><strong>Факультет</strong>: '+arContractInfo.PROPS.FACULTY+'</p>';
			 node.innerHTML += '<p><strong>Форма обучения</strong>: '+arContractInfo.PROPS.FORM+'</p>';

            node.appendChild(this.makeFormGroup('Направление',arContractInfo.PROPS.DIRECTION,true));
            node.appendChild(this.makeFormGroup('Номер договора',arContractInfo.PROPS.NUMBER,true));

            node.appendChild(this.makeFormGroup('Обучающийся',arContractInfo.PROPS.STUDENT_FIO,true));
            node.appendChild(this.makeFormGroup('Уровень образования',arContractInfo.PROPS.LEVEL,true));
            node.appendChild(this.makeFormGroup('Факультет',arContractInfo.PROPS.FACULTY,true));
            node.appendChild(this.makeFormGroup('Форма обучения',arContractInfo.PROPS.FORM,true));
			 */

			
            if ( typeof arPayments == 'object' ) {
            	this.paySystemBlockNode.innerHTML  += "<p>Взимается комиссия до 3% от суммы платежа в зависимости от платежной системы и банка, выпустившего платежную карту. </p>";

                /*
				var bxul = BX.create('ul', {props: {className: 'payment_type'}});

                var obj = this;
                $.each(arPayments,function(index,arPayment){

                    bxul.innerHTML += "<li style='list-style:none' data-percent='"+arPayment["PERCENT"]+"'><input id='payment"+index+"' name='payment_type' type='radio' value='"+arPayment["PERCENT"]+"'>&nbsp;<label for='payment"+index+"'>"+arPayment["NAME"]+"</label></li>";
                });

 				this.paySystemBlockNode.appendChild(bxul);
				*/
		...
		editFadePropsContent: function(node)
		{
			...
			if (!props || props.length == 0)
			{
				...
				if ( typeof arContractInfo.ID != undefined ){
		...
		makeFormGroup: function(title,value,readonly)
		{

            var div = BX.create('DIV', {props: {className: 'form-group'}});
            div.innerHTML  += '<label>'+title+'</label>';
            var readonlyAttr = readonly?'readonly=""':'';
            div.innerHTML  += '<input type="text" value="' + value +'" ' +readonlyAttr+'>';
            return div;
		...
		editPropsComment: function(propsNode)
		{
            ...
            propsCommentContainer.innerHTML += '<p><strong>Направление</strong>: '+arContractInfo.PROPS.DIRECTION+'</p>';
            propsCommentContainer.innerHTML += '<p><strong>Номер договора</strong>: '+arContractInfo.PROPS.NUMBER+'</p>';
            propsCommentContainer.innerHTML += '<p><strong>Обучающийся</strong>: '+arContractInfo.PROPS.STUDENT_FIO+'</p>';
            propsCommentContainer.innerHTML += '<p><strong>Уровень образования</strong>: '+arContractInfo.PROPS.LEVEL+'</p>';
            propsCommentContainer.innerHTML += '<p><strong>Факультет</strong>: '+arContractInfo.PROPS.FACULTY+'</p>';
            propsCommentContainer.innerHTML += '<p><strong>Форма обучения</strong>: '+arContractInfo.PROPS.FORM+'</p>';
		...
		editTotalBlock: function()
		{
			...
			{
                /*this.totalInfoBlockNode.appendChild(this.createTotalUnit('Комиссия:', window.payment_percent+'%', {percent: true}));*/
